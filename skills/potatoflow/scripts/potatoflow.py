...
def validate_plan(plan: dict[str, Any], existing_task_ids: set[str] | None = None) -> dict[str, Any]:
    ...
    for index, task in enumerate(tasks):
        ...
        string_list(task.get("steps"), f"tasks[{index}].steps")
        ...
        if not isinstance(priority, int) or priority not in range(1, 5):
            raise PotatoFlowError(f"tasks[{index}].priority must be between 1 and 4")
...
def command_today(args: argparse.Namespace) -> dict[str, Any]:
    ...
    tasks = [
        ...
        if task["scheduled_date"] == target_date and task["status"] != "cancelled"
    ]
...
def command_set_status(args: argparse.Namespace) -> dict[str, Any]:
    ...
    task["updated_at"] = now_iso()
    if args.note:
        task["notes"].append({"text": args.note.strip(), "created_at": now_iso()})
    add_event(
        state,
        "task.status_changed",
        task["id"],
        {"from": previous, "to": args.status, "reason": args.note or ""},
    )
    save_state(path, state)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    status_parser = sub.add_parser("set-status", help="Change a task status")
    status_parser.add_argument("--task-id", required=True)
    status_parser.add_argument("--status", required=True, choices=sorted(TASK_STATUSES))
    status_parser.add_argument("--note")
    add_data_argument(status_parser)
