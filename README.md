...
- 阶段负责分组，子任务是唯一可勾选的执行单位
- 执行步骤与完成标准只用于查看流程，不再拆成逐项勾选和逐项汇报
- 每条执行步骤可勾选并添加独立备注，任务底部保留统一结果汇报框
- 当前文件与完整 Git 历史均运行高置信度密钥扫描
...
`v0.7.0`：重构任务层级与执行页面。阶段只负责分组，子任务标题是唯一完成勾选；执行步骤和完成标准改为只读流程；任务统一填写一次结果汇报。新增本地文件与 Git 历史密钥扫描，并保留旧版逐步骤执行记录。
